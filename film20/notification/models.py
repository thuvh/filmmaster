...
def create_notice_type(label, display, description, default=2, type=0):
    """
    ...
    try:
        ...
        NoticeType(label=label, display=display, description=description, default=default, type=type).save()
...
def send_now(users, label, extra_context=None, on_site=True, priority=PRIORITY_DEFAULT):
    """
    ...
    for user in users:
        ...
        for medium in get_user_media(notice_type, user):
            medium.send_notice(user, notice_type, context)
...
def queue(users, label, extra_context=None, on_site=True, priority = PRIORITY_DEFAULT):
    ...
    for lang, userids in groupby(userids_w_lang, key=key):
        ...
        defer(_send_now, ids, label, extra_context, on_site, 
