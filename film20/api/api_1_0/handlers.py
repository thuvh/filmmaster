...
from film20.core.models import Object, Profile, Rating, Recommendation,\
...
from film20.core.search_helper import Search_Helper
...
class AnonPersonHandler(AnonymousBaseHandler):
    model = Person
    fields = ['name', 'surname', 'image', 'permalink', 
...
class AnonFilmHandler(AnonymousBaseHandler):
    model = Film
    fields = ('image', 'permalink', 'production_country_list', 'wishlist',
    ...
    @classmethod
    def auth_guess_rating(cls, film):
        if not film.pk: return None
        request = get_request()
        user = getattr(request, 'uri_user', None)
        if request.user.is_authenticated():
            if request.user.username == user:
                return cls.guess_rating(film)
            rating = user and rating_helper.guess_score(user, film.pk)
            return rating and rating.guess_rating

    @classmethod
    def guess_rating(self, film):
        if not film.pk: return None
        request = get_request()
        user = getattr(request, 'uri_user', None)
        if user:
            if hasattr(film, 'guess_rating'):
                # little optimization - some queries set this
...
class AnonUserFilmMetaHandler(AnonymousBaseHandler):
    ...
    def read(self, request, username, permalink):
        ...
        if date:
            ...
            film_channels = [FilmScreeningsChannel(channel=c, screenings=c.screenings) for c in screening_set.get_channels(film.id)]
        else:
            film_channels = None
        ...
        meta = {
            ...
            'basket': basket_item,
            'guess_rating': rating and rating.guess_rating,
...
class AnonUnratedFilmsHandler(AnonymousBaseHandler):
    ...
    def read(self, request, username):
        user = get_user(request, username)
        films = recom_helper.get_recently_popular_films_query(user.id, **film_filter_kwargs(request))
        return paginated_collection(request, films)
...
class SearchFilmHandler(AnonymousBaseHandler):
    def read(self, request):
        return search_helper.search_film(title = request.GET.get('phrase', ''))

class SearchPersonHandler(AnonymousBaseHandler):
    def read(self, request):        
        return search_helper.search_person_by_phrase(request.GET.get('phrase', ''))

class SearchUserHandler(AnonymousBaseHandler):
    def read(self, request):
        return search_helper.search_user_by_phrase(request.GET.get('phrase', ''))
...
class SearchHandler(AnonymousBaseHandler):
    def read(self, request):
        phrase = request.GET.get('phrase', '')
        return {
            'films':search_helper.search_film(title = phrase),
            'persons':search_helper.search_person_by_phrase(phrase),
            'users':search_helper.search_user_by_phrase(phrase),
        }
...
class UserShortReviewCommentsHandler(AnonBaseCommentsViewHandler):
    def get_object(self, username, permalink):
        return get_object_or_404(UserActivity, short_review__kind=ShortReview.REVIEW, username=username, short_review__object__permalink=permalink)
...
class AnonUserFilmShortReviewHandler(AnonymousBaseHandler):
    ...
    def resource_uri(self, review):
        return user_short_review_handler, [review.user.username, review.object.permalink]
    
    @uri
    def comments_uri(self, review):
        return user_short_review_comments_handler, [review.user.username, review.object.permalink]
...
def _get_activities(request, username=None, filter=None):
        ...
        if not (most_interesting_reviews or reviews or shorts or comments or links or checkins or follows):
            follows = reviews = shorts = comments = links = checkins = True

        comrades = P('comrades') | P('following')
        similar_taste=P('similar-taste')
        all_users = not comrades and not similar_taste

        favorites = P('favorites')
        
        
        _, activities = planet_helper.get_activities(request, 
                        all_activities, reviews, most_interesting_reviews, shorts, comments,
                        links, all_users, comrades, similar_taste, favorites, follows, checkins=checkins, username=username)
#        exclude = Q(activity_type=UserActivity.TYPE_COMMENT) & ~Q(comment__content_type__model__in=['post', 'shortreview'])
#        activities = activities.exclude(exclude)
        
        activities = activities.exclude(activity_type=UserActivity.TYPE_SHORT_REVIEW, film__isnull=True)
...
class AnonUserActivityHandler(AnonymousBaseHandler):
    ...
    def read(self, request, username, filter=None, id=None):
        if id:
            return get_object_or_404(UserActivity, username=username, pk=id)
    ...
    def resource_uri(cls, activity):
        return user_activity_handler, [activity.user.username, activity.id]
    ...
    def short_review_uri(cls, activity):
        return activity.short_review_id and (user_short_review_handler, [activity.username, activity.film_permalink])
    ...
    def comments_uri(cls, activity):
        if activity.activity_type in (UserActivity.TYPE_POST, UserActivity.TYPE_SHORT_REVIEW):
            return cls.object_uri(activity) + 'comments/'
...
def get_available_showtimes(request, view, username=None, town_id=None, channel=None):
    ...
    } for d in get_available_showtime_dates(request)))
...
class UserShowtimesHandler(AnonymousBaseHandler):
    ...
    def read(self, request, username, date=None):
        if not date:
            return get_available_showtimes(request, user_showtimes_handler, username=username)
        try:
...
class UserChannelShowtimesHandler(AnonymousBaseHandler):
    ...
    def read(self, request, username, date=None):
        if not date:
            return get_available_showtimes(request, user_channel_showtimes_handler, username=username)
        try:
            date = parse_date(request, date)
        except ValueError, e:
            return BAD_REQUEST(unicode(e))
...
class TownShowtimesHandler(AnonymousBaseHandler):
    def read(self, request, id, date=None):
        if not date:
            return get_available_showtimes(request, town_showtimes_handler, town_id=id)
        try:
            date = parse_date(request, date)
        except ValueError, e:
            return BAD_REQUEST(unicode(e))
...
class TownCinemaShowtimesHandler(AnonymousBaseHandler):
    def read(self, request, id, date=None):
        if not date:
            return get_available_showtimes(request, town_cinema_showtimes_handler, town_id=id)
        user = request.user        
        try:
            date = parse_date(request, date)
        except ValueError, e:
...
class TownCinemaHandler(AnonymousBaseHandler):
    def read(self, request, id):
        town = get_object_or_404(Town.objects_with_cinemas, pk=id)
        return paginated_collection(request, Channel.objects.in_town(town))
...
class FestivalHandler(AnonymousBaseHandler):
    ...
    fields = ('name', 'short_name', 'lead', 'body', 'tag', 'supertag', 'start_date', 
              ...
              'resource_uri', 'background_image', 'menu_header_image', 'menu_icon_image')

    @classmethod
    def background_image(cls, festival):
        image = festival.background_image
        return image and settings.MEDIA_URL + unicode(image) or ""

    @classmethod
    def menu_header_image(cls, festival):
        image = festival.menu_header_image
        return image and settings.MEDIA_URL + unicode(image) or ""

    @classmethod
    def menu_icon_image(cls, festival):
        image = festival.menu_icon_image
        return image and settings.MEDIA_URL + unicode(image) or ""
...
user_unrated_films_handler = Resource(UnratedFilmsHandler, AUTHORIZED)
