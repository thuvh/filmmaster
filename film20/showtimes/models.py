...
from film20.utils.db import QuerySet, FieldChangeDetectorMixin
...
class FilmOnChannel(models.Model):
    ...
    def __init__(self, *args, **kw):
        super(FilmOnChannel, self).__init__(*args, **kw)
        self.film_prev = self.film_id

    def save(self, *args, **kw):
        if self.match in (UNMATCHED, HAS_CANDIDATES) and self.film:
            self.match = MATCHED
        ret = super(FilmOnChannel, self).save(*args, **kw)
        if self.film_id != self.film_prev and self.match == MATCHED:
    ...
    def match_and_save(self):
        ...
        year = self.year
        directors = (d.strip() for d in (self.directors or '').split(','))
        tag = self.tag
        ...
        if self.imdb_code:
            ...
            LIMIT = 10
            query = Film.objects.all().distinct().order_by('-popularity')
            if year:
            ...
            films = query.filter(q1|q2)[:LIMIT]
            ...
            dfilms = filter(directors_match, films)
            dmatched = bool(dfilms)
            if dmatched:
                films = dfilms
...
class ScreeningCheckIn(FieldChangeDetectorMixin, models.Model):
    ...
    def notify(self):
        ...
        data = {
            ...
            'film':self.film,
            'channel':self.screening and self.screening.channel,
            'link':self.film.get_absolute_url(), #self.screening and self.screening.get_absolute_url(),
