...
def film_description(context, film, words=None, edit=False):
    short = False
    descr = unicode(strip_tags(film.description or gettext("Unfortunately, this film does not have a description, yet.")))
...
def film_others_ratings(request, film=None):
    film = film or Film.objects.get(id=request.GET['film_id'])
    return {
        'film':film,
    }

@register.inclusion_tag('movies/rating/rating_progress.html', takes_context=True)
...
def film_ratings_form(context, film, user, mark_as_seen = False, edit=False):
    form = FilmRatingForm(None, film=film, user=user, mark_as_seen=mark_as_seen, edit=edit)
    return {
        'user':user,
        'film':film,
        'form':form,
...
def single_film_ratings_form(context, film, user, mark_as_seen = False):
    """
        Displays film ratings form but with rate more link instead of rate next
    """
    return film_ratings_form(context, film, user, mark_as_seen, edit=True)

@register.inclusion_tag('movies/rating/ratings_form_simple.html', takes_context=True)
def film_ratings_form_simple(context, film, user, mark_as_seen = False):
    form = SimpleFilmRatingForm(None, film=film, user=user, mark_as_seen=mark_as_seen)
    return {
        'user':user,
        'film':film,
        'form':form,
...
@register.inclusion_tag('movies/rating/add_edit_film_rate_form.html',
        takes_context=True)
def add_edit_film_rate_form(context, film, mark_as_seen = False):
    request = context.get('request')
    form = SimpleFilmRatingForm(None, film=film, user=request.user,
...
def film_ratings_form_special(context, film, user, mark_as_seen = False):
    form = SimpleFilmRatingForm(None, film=film, user=user, mark_as_seen=mark_as_seen)
    return {
        'user':user,
        'film':film,
