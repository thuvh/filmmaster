...
def get_image_path(the_object, size_x, size_y):
    if the_object.type == the_object.TYPE_PERSON:
        default_path = settings.DEFAULT_PHOTO
    else:
        default_path = settings.DEFAULT_POSTER

    path = unicode(the_object.hires_image) or ''
    if not _exists(path):
        path = unicode(the_object.image) or ''
    if not _exists(path):
        path = default_path
