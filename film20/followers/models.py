...
class Followers(models.Model):
    ...
    def save(self, *args, **kw):
        ...
        #                                                  v hack for #FLM-1572
        if first_save and self.status==Followers.FOLLOWING and self.to_user.username <> 'filmaster':
            self.save_activity()
...
# Automatically add "filmaster" to following users 
#    after creating new account.
def user_post_save( sender, instance, created, *args, **kw ):
    if created:
        try:
            if instance.username <> 'filmaster': # ...
                user_to_follow = User.objects.get( username='filmaster' )
                instance.followers.follow( user_to_follow )
        except User.DoesNotExist:
           logger.error( "user `filmaster` doesn't exist [SKIPPING] " )
