...
from .forms import LocationForm, ChangePasswordForm, SetPasswordForm, ProfileForm, ConsumerForm, AvatarForm, AvatarCropForm
        ...
        if "criticker" in request.POST:
            ...
            if criticker_form.is_valid():
                ...
                ratings_list = parse_criticker_votes(xml_file=ratings_file,
                    score_convertion=scoring, import_reviews=reviews)
                save_ratings_db(request.user, ratings_list, ImportRatings.CRITICKER,
                    overwrite=overwrite)
            ...
            if imdb_form.is_valid():
                ...
                save_ratings_db(request.user, ratings_list, ImportRatings.IMDB, overwrite=overwrite)
        elif "filmweb" in request.POST:
            filmweb_form = ImportFilmwebRatingsForm(request.POST)
            if filmweb_form.is_valid():
                vote_history = fetch_votes_from_filmweb(filmweb_form.cleaned_data.get('filmweb_user'))
                criticker_xml = parse_filmweb_votes(vote_history)
                ratings_list = parse_criticker_votes(xml_string = criticker_xml, score_convertion=SCORE_EXACT)
                overwrite = filmweb_form.cleaned_data['overwrite']
                save_ratings_db(request.user, ratings_list, ImportRatings.FILMWEB, overwrite=overwrite)
...
def associations(request):
    ...
    class Association(object):
        ...
        def is_associated(self):
            return bool(self.service.get_user_id(self.request.user))
    ...
    oauth_associations = (Association(service, request) for service in OAUTH_SERVICES)
