...
def ddebug(str):
    if DEBUG_DETAILED == True:
        if DEBUG_USING_LOGGING:
            logger.debug("[DDEBUG] " + unicode(str))
        else:
...
def count_guessed_rate(user, film):
    ratings = list(Rating.objects.filter(type=1, rating__isnull=False,
        user=user).values('film', 'rating'))

    if len(ratings) < MIN_USER_RATES:
        return None

    average_rate, average_weight = AVERAGE_RATE, AVERAGE_WEIGHT
    film_rate, film_weight = 0.0, 0.0
    film_avg = FilmRanking.objects.filter(film=film).order_by(
            "-number_of_votes")
    if len(film_avg) > 0:
        film_rate = float(film_avg[0].average_score)
        film_weight = FILM_WEIGHT * pow(
                float(film_avg[0].number_of_votes / FILM_RELIABILITY_BAR), 0.3)

    user_rate, user_weight = 0.0, 0.0
    for rating in ratings:
        user_rate += rating['rating']
    if len(ratings) > 0:
        user_rate /= len(ratings)
        user_weight = USER_WEIGHT * pow(
                float(len(ratings) / USER_RELIABILITY_BAR), 0.3)

    scores = {}
    comparators = list(FilmComparator.objects.filter(main_film=film).values(
                                                'compared_film', 'score'))
    for comp in comparators:
        scores[comp['compared_film']] = float(comp['score'])

    sum_weights = 0.0
    sum_rate = 0.0
    for rating in ratings:
        if rating['film'] in scores:
            weight = scores[rating['film']]
            sum_weights += weight
            sum_rate += float(rating['rating']) * weight
