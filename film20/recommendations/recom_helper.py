...
#        query = list(query)
    ...
    def get_recently_popular_films_query(self, user_id=None, rated_films=None, tags=None, year_from=None, year_to = None, related_director = None, related_actor = None, popularity = None, limit = 100):
        """
    ...
    def get_best_psi_films_queryset(self, user_id=None, tags=None, year_from=None, year_to = None, related_director = None, related_actor = None, popularity = None, limit = None, offset = None):
        return self.get_best_psi_films_queryset_alg2(user_id, tags, year_from, year_to, related_director, related_actor, popularity, limit, offset)

    def get_best_psi_films_queryset_alg2(self, user_id=None, tags=None, year_from=None, year_to = None, related_director = None, related_actor = None, popularity = None, limit = None, offset = None):
        """
            Gets the best PSI movies for user using the ALG2 algorithm.
            (described: http://filmaster.org/display/DEV/New+recommendation+engine)
        """
        logger.debug("user_id: " + unicode(user_id))
        psi_qset = (
            Q(film_recommendation__user = user_id)
        )

        top_films = Film.objects.filter(psi_qset).select_related('film')
        top_films = top_films.extra(
            select={
                    'guess_rating': 'core_recommendation.guess_rating_alg2',
            },
            where=[
                'core_film.parent_id NOT IN (SELECT film_id FROM core_rating WHERE user_id=%i AND type=1 AND rating IS NOT NULL)' % user_id,
                'core_film.parent_id NOT IN (SELECT film_id FROM filmbasket_basketitem WHERE user_id=%i)' % user_id,
            ],
        )
        ...
        top_films = top_films.order_by("-film_recommendation__guess_rating_alg2")
        if limit:
            top_films = top_films[:limit]

        if offset:
            top_films = top_films[offset:]
    ...
    def get_best_psi_films(self, limit=100, user_id=None, rated_films=None):
        """
            Gets the X best PSI movies and removes the movies recently viewed by the user (in session)
        """
        top_films = self.get_best_psi_films_queryset(user_id=user_id)
    
        # exclude all in session
        if rated_films:
            excluded_qset = (
                Q(parent__in=rated_films)
            )
            top_films = top_films.exclude(excluded_qset)
        
        top_films = list(top_films[:limit])
        logger.debug("Best PSI movies (sliced): ")
        logger.debug(top_films)
    ...
    def count_ratings(self, user_id):
        ...
        result = get_cache(CACHE_USER_RATINGS_COUNT, unicode(user_id))
        ...
        # store in cache
        set_cache(CACHE_USER_RATINGS_COUNT, unicode(user_id), ratings_count)
    ...
    def get_ranking(self, type=Rating.TYPE_FILM, tags=None, year_from=None, year_to = None, related_director = None,
                    ...
                    limit = None, offset = None, min_num_votes = None, festival = None, exclude_production_country=None):
        """
        ...
        if min_num_votes == None:
            ...
            # this is user-entered
                ...
                min_num_votes = FilmRanking.MIN_NUM_VOTES_FILM
            else:
