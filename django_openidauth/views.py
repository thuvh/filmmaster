...
import time, hashlib as md5, datetime
...
#mrk
    ...
    if 'openid_url' in request.POST:
        ...
        def openid_failure(request,message):
          return message
        ret = consumer_views.begin(request, redirect_to=post_login_redirect, on_failure=openid_failure)
    ...
    if request.GET.get('direct') and request.openids and \
        ...
        messages.append(_('%s has been associated with your account') % escape(new_openid))
    ...
    for key in request.POST.keys():
        if key in add_hashes:
            ...
            if openid not in associated_openids:
                ...
                messages.append( _('%s has been associated with your account') % escape(openid) )
        if key in del_hashes:
            ...
            if openid in associated_openids:
                ...
                messages.append(_('%s has been removed from your account') % escape(openid))
