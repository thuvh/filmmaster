...
class TypeMapper(dict):
    def __init__(self, *args, **kw):
        super(TypeMapper, self).__init__(*args, **kw)
        self.model_mapper = { }

    def __setitem__(self, key, value):
        model, anon = value
        if model:
            self.model_mapper[value] = key
        return super(TypeMapper, self).__setitem__(key, value)
    
    def already_registered(self, model, anon):
        return self.model_mapper.get((model, anon))
...
class HandlerMetaClass(type):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        mapper = new_cls.mapper()
        
        if hasattr(new_cls, 'model'):
            if mapper.already_registered(new_cls.model, new_cls.is_anonymous):
            ...
            mapper[new_cls] = (new_cls.model, new_cls.is_anonymous)
        else:
            mapper[new_cls] = (None, new_cls.is_anonymous)
