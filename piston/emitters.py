...
class Emitter(object):
    ...
    def construct(self):
        ...
        """
        self.path = []
        def _any(thing, fields=None, name=None):
            """
            Dispatch, all types are routed through here.
            """
            ret = None
            if name: self.path.append(name)
            ...
            if name: self.path.pop()
            return ret

        def _fk(data, field, name=None):
            """
            Foreign keys.
            """
            return _any(getattr(data, field.name), name=name)
        ...
        def _m2m(data, field, fields=None):
            ...
            """
            return [ _model(m, fields, name=field.name) for m in getattr(data, field.name).iterator() ]
        ...
        def _model(data, fields=None):
            ...
            if handler or fields:
                ...
                if handler:
                    ...
                    """
                    fields = getattr(handler, 'get_fields', lambda e:handler.fields)(self)
                    ...
                    if f.serialize and not any([ p in met_fields for p in [ f.attname, f.name ]]):
                        if not f.rel:
                            if f.attname in get_fields:
                                ret[f.attname] = _any(v(f), name=f.attname)
                                get_fields.remove(f.attname)
                        else:
                            if f.attname[:-3] in get_fields:
                                ret[f.name] = _fk(data, f, name=f.name)
                        ...
                        if inst:
                            if hasattr(inst, 'all'):
                                ...
                                if len(inspect.getargspec(inst)[0]) == 1:
                                    ret[model] = _any(inst(), fields, name=model)
                            else:
                                ret[model] = _model(inst, fields, name=model)

                    elif maybe_field in met_fields:
                        # Overriding normal field which has a "resource method"
                        # so you can alter the contents of certain fields without
                        # using different names.
                        ret[maybe_field] = _any(met_fields[maybe_field](data), name=maybe_field)
                        ...
                        if maybe is not None:
                            if callable(maybe):
                                if len(inspect.getargspec(maybe)[0]) <= 1:
                                    ret[maybe_field] = _any(maybe(), name=maybe_field)
                            else:
                                ret[maybe_field] = _any(maybe, name=maybe_field)
                        else:
                            ...
                            if handler_f:
                                ret[maybe_field] = _any(handler_f(data), name=maybe_field)

            else:
                for f in data._meta.fields:
                    ret[f.attname] = _any(getattr(data, f.attname), name=f.attname)

                fields = dir(data.__class__) + ret.keys()
                add_ons = [k for k in dir(data) if k not in fields]

                for k in add_ons:
                    ret[k] = _any(getattr(data, k), name=k)

            # resouce uri
            if self.in_typemapper(type(data), self.anonymous):
                handler = self.in_typemapper(type(data), self.anonymous)
                if hasattr(handler, 'resource_uri'):
                    uri = handler.resource_uri(data)
                    if isinstance(uri, basestring):
                        ret['resource_uri'] = uri
                    else:
                        url_id, fields = uri
                        try:
        ...
        def _dict(data, fields=None):
            ...
            Dictionaries.
            """
            return dict([ (k, _any(v, fields, name=k)) for k, v in data.iteritems() ])

        # Kickstart the seralizin'.
        return _any(self.data, self.fields)

    def in_typemapper(self, model, anonymous):
        return self.typemapper.already_registered(model, anonymous) or \
               self.typemapper.already_registered(model, True)
